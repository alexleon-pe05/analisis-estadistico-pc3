...
# ============================================================
# DATOS - PROBLEMA #2 - GreenBuild Smart Towers
# ============================================================
...
p = 4  # tamaño del cuadrado Latino

# Cuadrado Latino: fila=Zona, columna=Estacion
cuadrado = [
    ["A","B","C","D"],   # Norte:  Verano=A  Otono=B  Invierno=C  Primavera=D
    ["B","C","D","A"],   # Sur:    Verano=B  Otono=C  Invierno=D  Primavera=A
    ["C","D","A","B"],   # Este:   Verano=C  Otono=D  Invierno=A  Primavera=B
    ["D","A","B","C"],   # Oeste:  Verano=D  Otono=A  Invierno=B  Primavera=C
]

y_mat = [
    [12.03, 16.07, 20.76, 14.21],   # Norte
    [17.34, 20.59, 15.48, 13.03],   # Sur
    [19.70, 13.08, 12.07, 16.24],   # Este
    [14.29, 11.83, 17.38, 20.74],   # Oeste
]

NIVELES_FIL  = ["Norte", "Sur", "Este", "Oeste"]
NIVELES_COL  = ["Verano", "Otono", "Invierno", "Primavera"]
NIVELES_TRAT = ["A", "B", "C", "D"]
...
for i in range(p):
    for j in range(p):
        fila_col.append(NIVELES_FIL[i])
        col_col.append(NIVELES_COL[j])
        trat_col.append(cuadrado[i][j])
        y_col.append(y_mat[i][j])

df = pd.DataFrame({
    NOMBRE_FIL:  pd.Categorical(fila_col,  categories=NIVELES_FIL),
    NOMBRE_COL:  pd.Categorical(col_col,   categories=NIVELES_COL),
    NOMBRE_TRAT: pd.Categorical(trat_col,  categories=NIVELES_TRAT),
    'Y': y_col
})
...
print(f"\nFt Zona      ({gl_F},{gl_E}) = {ft_F:.4f} | Fc={Fc_F:.4f} -> {'SIGNIFICATIVO' if Fc_F>ft_F else 'no sig.'}")
print(f"Ft Estacion  ({gl_C},{gl_E}) = {ft_C:.4f} | Fc={Fc_C:.4f} -> {'SIGNIFICATIVO' if Fc_C>ft_C else 'no sig.'}")
print(f"Ft Sistema   ({gl_T},{gl_E}) = {ft_T:.4f} | Fc={Fc_T:.4f} -> {'SIGNIFICATIVO' if Fc_T>ft_T else 'no sig.'}")

# ============================================================
# TUKEY MANUAL - Sistema (tratamiento)
...
print(f"  CM_E={cm_E_v:.6f}  gl_E={gl_E}  n_nivel=p={p}")
